...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  try {
    if (!GITHUB_ORG) return json(500, { success: false, error: 'GITHUB_ORG not configured', requestId, timestamp: now() });
    const name = event.queryStringParameters?.name;
    const invalid = validateName(name);
    if (invalid) return json(400, { success: false, error: invalid, requestId, timestamp: now() });
    const token = await getGithubToken();
    const resp = await fetch(https://api.github.com/orgs//repos, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ name, visibility: 'private', auto_init: true })
    });
    ...
    if (!resp.ok) {
      ...
      return json(resp.status, { success: false, error: errorMsg, details: { githubStatus: resp.status, githubMessage: data?.message, errors: data?.errors }, requestId, timestamp: now() });
    }
    return json(201, { success: true, repo: { name: data?.name, html_url: data?.html_url }, requestId, timestamp: now() });
  } catch (err: unknown) {
